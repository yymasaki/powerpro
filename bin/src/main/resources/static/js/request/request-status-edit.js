$(function(){
	...
	$(".personality-skill").ready(function(){
		...
		let replacedUrl;
		if(url.includes('/do')){
			replacedUrl = url.replace('/request/status/edit/do', '/get_had_personality');				
		}else{
			replacedUrl = url.replace('/request/status/edit', '/get_had_personality');						
		}
		$.ajax({
			url : replacedUrl,
			type : "GET",
			...
			// 全ての性格リストを取得.
			let personalitySkillList = document.getElementsByClassName('personality-skill');
	...
	// 管理者コメントが1文字も入力されていなければ申請ボタン非活性化
	 ...
	 // エンジニアスキルの点数が101以上なら100にする
	 $('.engineer-skill-score').on('input', function() {
		 let value = $(this).val();
         // 01,02等のパターン
         const numberPattern = /^[0]{1}\d{1}$/;
         //a,あ,!等の数字以外のパターン
       ...
       　// 101以上を入力したら100にする
         ...
         if(this.value.match(numberPattern) || value < 0){
